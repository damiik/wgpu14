...
fn create_vertices(
    ...
    let v: [f32; 3] = [0.0, 1.0, 0.0];
    for i in 0..pos.len() {
        data.push(common::vertex(pos[i], normal[i], v));
    }
    data.to_vec()
}
...
fn main() {
    ...
    let light_data = common::light([1.0, 0.0, 0.0], 0.1, 0.1, 0.6, 0.3, 0);
    common::run(&vertex_data, light_data, "torus", "xxx");
}
